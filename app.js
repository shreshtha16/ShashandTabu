...
async function setupMessaging(user){const button=document.getElementById("enableNotifications");if(!("Notification" in window)||!navigator.serviceWorker){button.disabled=true;return}const registerNotifications=async()=>{if(FCM_VAPID_KEY.startsWith("REPLACE_")){showPrivateError({message:"Add your Firebase Web Push certificate key in app.js first."});return}try{const registration=await navigator.serviceWorker.register("firebase-messaging-sw.js");const token=await getToken(messaging,{vapidKey:FCM_VAPID_KEY,serviceWorkerRegistration:registration});await setDoc(doc(db,"notificationTokens",user.uid),{uid:user.uid,token,updatedAt:serverTimestamp()});button.textContent="notifications enabled"}catch(err){showPrivateError(err)}};button.onclick=async()=>{const permission=await Notification.requestPermission();if(permission==="granted")await registerNotifications()};if(Notification.permission==="granted")await registerNotifications();onMessage(messaging,payload=>{const title=payload.notification?.title||"A note from us";new Notification(title,{body:payload.notification?.body||"You have a new note."})})}
